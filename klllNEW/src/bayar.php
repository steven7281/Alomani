...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $kelurahan = $_POST['kelurahan_text'] ?? '';
    ...
    $query_cart = mysqli_query($conn, "SELECT k.*, p.nama_barang, p.harga FROM tb_keranjang k 
    ...
    if (!$query_cart || mysqli_num_rows($query_cart) === 0) {
        echo "<script>alert('Keranjang kosong atau gagal mengambil data'); window.location='keranjang.php';</script>";
        exit();
    }
    ...
    $query_order = "INSERT INTO tb_order (id_user, nama, nohp, provinsi, kota, kecamatan, kelurahan, kode_pos, alamat, grand_total, status_pembayaran) 
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    if (mysqli_stmt_execute($stmt)) {
        ...
        mysqli_query($conn, "UPDATE tb_keranjang SET status = 1 WHERE id_user = '$id_user' AND status = 0");
        ...
        try {
            ...
            $update_query = "UPDATE tb_order SET snap_token = ?, status_pembayaran = 'Pending' WHERE id_order = ?";
            $stmt_update = mysqli_prepare($conn, $update_query);
            mysqli_stmt_bind_param($stmt_update, 'si', $snapToken, $order_id);
            mysqli_stmt_execute($stmt_update);

            mysqli_query($conn, "CREATE TABLE IF NOT EXISTS tb_order_detail (
            ...
            $query_detail = "INSERT INTO tb_order_detail (id_order, ongkir, total_harga) VALUES (?, ?, ?)";
            $stmt_detail = mysqli_prepare($conn, $query_detail);
            mysqli_stmt_bind_param($stmt_detail, 'idd', $order_id, $ongkir, $grand_total);
            ...
            echo "<script>alert('Error Midtrans: " . $e->getMessage() . "'); window.history.back();</script>";
        }
    } else {
        echo "<script>alert('Error menyimpan order: " . mysqli_error($conn) . "'); window.history.back();</script>";
    }
}
?>
    ...
    <main class="container">
        ...
        <div class="row">
            ...
            <form class="col-md-6 form-section" method="POST">
                ...
                <button type="submit" class="cart-modern-btn-main">BAYAR</button>
            </form>
